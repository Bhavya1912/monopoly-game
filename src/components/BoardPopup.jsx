...
function SwapPanel({ props, myIdx, rawPlayers, onSwap, onDismiss }) {
    ...
    return (
        <div
            ...
            style={{ top: "50%", left: "50%", transform: "translate(-50%,-50%)", position: "fixed", zIndex: 100, maxWidth: 300, maxHeight: "80vh", overflowY: "auto" }}
        >
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 3, marginBottom: 10 }}>
                {myProps.map(([id]) => {
                    ...
                    return (
                        <button key={id} onClick={() => setMyPick(+id)}
                            style={{ padding: "4px 8px", border: `2px solid ${myPick === +id ? "#14532d" : "#d6d3d1"}`, borderRadius: 6, cursor: "pointer", fontSize: 12, background: myPick === +id ? "#dcfce7" : "#fff" }}>
                            {s.name}
                        </button>
                    );
                })}
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 3, marginBottom: 10 }}>
                {theirProps.map(([id, p]) => {
                    const s = SPACES[+id]; if (!s) return null;
                    return (
                        <button key={id} onClick={() => setTheirPick(+id)}
                            style={{ padding: "4px 8px", border: `2px solid ${theirPick === +id ? PLAYER_COLORS[p.owner] : "#d6d3d1"}`, borderRadius: 6, cursor: "pointer", fontSize: 12, background: theirPick === +id ? `${PLAYER_COLORS[p.owner]}22` : "#fff" }}>
...
export default function BoardPopup({
    modal, players, myIdx, isMyTurn, onBuy, onPass, onDismiss,
                ...
                {isMe ? (
                    <>
                        ...
                        <div style={{ display: "flex", gap: 8 }}>
                            ...
                            <button onClick={onPass} style={btnStyle("#dc2626", true)}>Pass ✗</button>
                        </div>
                    </>
                ) : (
    ...
    if (modal.type === "build") {
        ...
        return (
            <div className="board-popup pop-in"
                ...
                <div style={{ fontSize: 12, color: "#78716c", marginBottom: 10 }}>Build cost: ${space.houseCost}/house</div>
                <div style={{ display: "flex", gap: 8 }}>
                    {canBuild && (
                    ...
                    {Object.entries(props).map(([id, p]) => {
                        ...
                        return (
                            <button key={id} onClick={() => onSteal(+id)}
                                style={{ display: "flex", alignItems: "center", gap: 6, padding: "6px 10px", background: `${PLAYER_COLORS[p.owner]}22`, border: `1px solid ${PLAYER_COLORS[p.owner]}`, borderRadius: 6, cursor: "pointer", fontSize: 12 }}>
