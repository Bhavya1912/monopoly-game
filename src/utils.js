import { SPACES, COLOR_GROUPS, PLAYER_COLORS, PLAYER_TOKENS, DEFAULT_SETTINGS } from "./constants";
...
export const safePlayers = gs => Array.isArray(gs?.players) ? gs.players : [];
export const safeProps = gs => gs?.properties && typeof gs.properties === "object" ? gs.properties : {};
export const safeLog = gs => Array.isArray(gs?.log) ? gs.log : [];
export const safeDice = gs => Array.isArray(gs?.dice) && gs.dice.length === 2 ? gs.dice : [1, 1];
export const safeSettings = gs => ({ ...DEFAULT_SETTINGS, ...(gs?.settings || {}) });
...
export function estimatePropertyRent(spaceId, prop, props) {
    ...
    if (space.type === "railroad") {
        const count = Object.entries(props)
            .filter(([k, v]) => v && v.owner === prop.owner && SPACES[+k]?.type === "railroad").length;
...
export function freshGameState(playerCount, settings = {}, aiPlayers = [], aiConfigs = {}) {
    return {
