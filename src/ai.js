...
export const AI_PERSONALITY = {
    aggressive: { buyMod: +0.2, buildMod: +0.25, cashMod: -0.15 },
    conservative: { buyMod: -0.2, buildMod: -0.2, cashMod: +0.25 },
    monopolist: {
        buyMod: +0.1,
        buildMod: +0.15,
        cashMod: 0,
        monopolyGroupBonus: 0.4,
    },
    random: { buyMod: 0, buildMod: 0, cashMod: 0, randomFactor: 0.6 },
};
...
export function aiShouldBuy(player, space, props) {
    if (!space?.price) return false;
    const cfg = player.aiConfig || {
        difficulty: "medium",
    ...
    const alreadyOwn = group.filter(
        (id) => props[id]?.owner === player.id,
    ).length;
    ...
    const opponentNearMono = group.some(
        (id) =>
            props[id] &&
            props[id].owner !== player.id &&
            group.filter((gid) => props[gid]?.owner === props[id]?.owner).length >=
            group.length - 1,
    );
    if (opponentNearMono) score += 0.2 * diff.riskTolerance;
