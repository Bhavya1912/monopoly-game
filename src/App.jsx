...
// ─── Data constants ──────────────────────────────────────────────────────────
import {
  STYLE, SPACES, COLOR_GROUPS, CHANCE_CARDS, COMMUNITY_CARDS, FREE_PARKING_EVENTS,
  PLAYER_COLORS, PLAYER_TOKENS, CELL_POSITIONS, DEFAULT_SETTINGS, COLOR_LABELS,
} from "./constants";

// ─── Utility helpers ─────────────────────────────────────────────────────────
import {
  clamp, generateCode,
  safePlayers, safeProps, safeLog, safeDice, safeSettings,
  estimateAssetValueForPlayer, estimatePropertyRent, freshGameState,
} from "./utils";

// ─── AI logic ────────────────────────────────────────────────────────────────
import {
  AI_DIFFICULTY, AI_PERSONALITY,
  aiShouldBuy, aiShouldBuild, aiPickPropertyToSell, aiJailStrategy,
} from "./ai";
...
export default function App() {
  ...
  useEffect(()=>{
    ...
    const unsub=onValue(gameRef,snap=>{
      ...
      if (data.players && !Array.isArray(data.players)) data.players = Object.values(data.players);
